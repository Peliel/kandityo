...
\begin{proof}
    ...
    \end{equation*}
    Huomataan lisäksi, että $\D^4=\D^2\times \D^2$ (samoin kuin $\D^2=\D^1\times \D^1$). Tällöin koska $\S^3\approx\partial\D^4$,
    \begin{align*}
...
\end{maaritelma}
Edellä on jo esitetty, että $\S^3=\TT^2\sqcup\TT^2$, kun $\TT^2$ ovat triviaaleja toruksia, mutta on mahdollista rakentaa $\S^3$ myös vääntyneistä toruksista, minkä esittelemiseen loppu tästä osiosta käytetään. Yleisempi tapaus epätriviaalisti kuiduttuneesta avaruudesta on Seifert-kuidutus.
\begin{maaritelma}
...
\begin{lause}
    Olkoot $k_1,k_2\geq2$ kokonaislukuja siten, että $\syt(k_1,k_2)=1$. On olemassa kuidutus $\pi_{k_1,k_2}:\S^3\to\S^2$, jolla on täsmälleen kaksi erityiskuitua kertoimilla $k_1,k_2$.
...
\begin{proof}[Lauseen~\ref{lause:s3-kuidutus} topologinen todistus]
    ...
    Olkoon $\S^1=\partial\D^2$ edellisen levyn reuna. Tällöin $\tilde{\pi}^{-1}[\S^1]\approx\T\subset E$. Toruksen $\T$ poikkilevy on minkä tahansa $x\in\S^1$ kuitu $\tilde{\pi}^{-1}(x)$; siis $\T$:n ympäryspiiri on $\TT$:n pituuspiiri ja toisin päin! Toisaalta silti $\T=\partial\TT$, mutta koska pituus- ja ympäryspiirit ovat vastakkain, $\T$:n ``sisus'' on $\TT$:n ulkopuoli.
    ...
    Jos $x_i\in\S_i^1$, niin määritellään $Y_i=\tilde{\pi}^{-1}(x_i)$. Tällöin $P_i:=\S_i^1$ ja $Y_i$ muodostavat homologiaryhmän $H_1(\T_i)$ kannan, sillä $P_i$ vastaa toruksen $\T_i$ pituuspiiriä ja $Y_i$ ympäryspiiriä.
    ...
    \begin{align*}
        ...
        &\Rightarrow\quad d=1.
    \end{align*}
    Olkoon $C_i=k_iY_i+n_iP_i\subset\T_i$ käyrä. Koska $\syt(k_i,n_i)=1$ pätee, $C_i$ kiertää molemmat toruksen homologia-akselit, jotka kummatkin estävät $C_i$:n typistämisen pisteeksi. $C_i$:n $H_1$-homologialuokka on
    \begin{align*}
        [C_i]&=[k_iY_i+n_iP_i] \\
        &=k_i[Y_i]+n_i[P_i]
    \end{align*}
    Jos pätisi $[C_i]=0$, niin tällöin $C_i$ eläisi hyvin mahdollisesti $\S^3$:ssa, sillä $\S^3$ on yhdesti yhtenäinen avaruus, jossa ei ole reikiä. Siispä strategiamme on liimata $\T_i$ täytetyn toruksen $\TT_i$ reunaan vääntämällä torusta $\TT_i$ siten, että
    \begin{equation*}
        [k_iY_i+n_iP_i]=[0].
    \end{equation*}
    Jos $\T_i$ ja $\TT_i$ liimataan yhteen pintojaan pitkin \emph{ilman vääntöä}, osuu pituuspiiri $P_i\subset\T_i$ jo valmiiksi ympäryspiirin $\mathbf{Y}_i\subset\TT_i$ päälle. Kuitenkin tämä triviaali liimaus jättää ympäryspiirin $Y_i\subset\T_i$ osumaan pituuspiirin $\mathbf{P}_i\subset\TT_i$ päälle ja $[\mathbf{P}_i]\ne[0]$. Tällöin $H_1$-homologialuokka olisi
    \begin{align*}
        [C_i]&=k_i[Y_i]+n_i[P_i] \\
        &=k_i[\mathbf{P}_i]+n_i[\mathbf{Y}_i] \\
        &=k_i\Z+n_i[0] \\
        &=k_i\Z\ne[0]
    \end{align*}
    Määritellään tämän liimauksen sijasta diffeomorfismi $f:\TT_i\to\T_i$ siten, että $f(\mathbf{Y}_i)=C_i$. Tärkeää on, että $f$ tekee liimauksen molemmille toruspareille $i=1,2$ samanaikaisesti. Koska tämä vaikeasti visualisoitava diffeomorfismi identifioi $C_i$:n ja ympäryspiirin $\mathbf{Y}_i$ toisikseen, tulosavaruudessa $C_i$-käyrän $H_1$-homologialuokka on
    \begin{align*}
        [C_i]&=[k_iY_i+n_iP_i] \\
        &=[\mathbf{Y}_i]=[0].
    \end{align*}
    Nyt kun käsitys oikeaoppisesta liimauksesta on selvillä, on aika liimata torukset yhteen. $\TT_i$ pitää ensin kovertaa avaruudesta, jotta sen vääntäminen ei väännä muuta avaruutta pieleen. Olkoon 
    \begin{align*}
        E\degree&=E\setminus\text{int}\left(\TT_1\cup\TT_2\right)
    \end{align*}
    kyseinen ``koverrettu'' avaruus. Seuraavaksi $E\degree$ täytetään toruksilla $\TT_i$, mutta siten, että toruksia väännetään oikein ja ne liimataan edellä määritellyn funktion $f$ osoittamalla tavalla toruksiin $\T_i$. Määritellään täksi avaruudeksi
    \begin{align*}
        X=(E\degree\sqcup\TT_1\sqcup\TT_2)/\sim,
    \end{align*}
    missä $x\sim y$, jos $f(x)=y$ ($x,y\in E$). Liimausoperaatio on tällöin kuvaus
    \begin{align*}
        L:E\degree\sqcup\TT_1\sqcup\TT_2\to X.
    \end{align*}
    Määritellään nyt kuvaus $\pi:X\to\S^2$ siten, että
    \begin{itemize}
        \item kaikille $x\in E\degree$ pätee $\pi\circ L(x)=\tilde{\pi}(x)$ ja
        \item kaikille $x\in\T_i$ pätee $\pi\circ g(x)=P(x)$, missä $P$ on projektio
        \begin{align*}
            P:\D_i^2\times\S^1&\to\D_i^2 \\
            (d,s)&\mapsto(d).
        \end{align*}
    \end{itemize}
    Todistetaan lopuksi, että $X=\S^3$. Edellä jo todistettiin, että käyrän $C_i$ homologialuokka on $X$:ssä triviaali.

    \textcolor{red}{Tässä on looginen hyppy. Miten pääsen $C_i$:n homologialuokan triviaaliudesta eteenpäin? Mikä on tämä puuttuva palanen? Alla oleva tieto on kielimallilta, koska en osannut netistä hakea sopivaa tietoa asiaan.}
    
    Tämän työn sisällön kannalta kohtuullisilla esitietovaatimuksilla ei ole mahdollista perustella, että $X$:n homologiaryhmän $H_1$ triviaalius on lisäksi \emph{riittävä} ehto siihen, että liimauksen jälkeinen avaruus olisi $\S^3$. On kuitenkin olemassa tuloksia, joiden mukaan avaruutta $Z\ne\S^3$, joka on homologinen 3-palloon, ei ole mahdollista konstruoida vain kahdesta niinkin yksinkertaisesta kappaleesta kuin toruksesta. Siispä todellakin $X=\S^3$.

    \textcolor{red}{Jos $n_i$ oltaisiin valittu toisin, olisiko $X'$ ollut sitten erilainen? Vai millainen diffeomorfismi $X=X'$ olisi olemassa?}

    Voidaan siis todeta, että $\pi$ on kaivattu kuvaus $\pi_{k_1,k_2}$ ja erityiskuidut, joiden moninkerrat ovat $k_i$, ovat torusten $\TT_i$ keskiakselit.
\end{proof}