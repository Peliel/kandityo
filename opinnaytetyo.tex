...
\tikzset{
    % 1) Nœud plein (cercle noir)
    solid node/.style={circle, draw=black, fill=black, inner sep=2pt},
    % 2) Nœud vide (cercle blanc)
    empty node/.style={circle, draw=black, fill=white, inner sep=2pt},
    % 3) Nœud invisible
    placeholder/.style={circle, draw=white, fill=white, inner sep=2pt},
    % 4) Nœud rectangulaire pouvant contenir un symbole
    rectangle node/.style={
        rectangle, 
        draw=black, 
        fill=white, 
        inner sep=4pt,       % Marge interne autour du texte
        minimum size=15pt,   % Taille minimale pour garder une uniformité
        align=center         % Centrage du texte
    },
    % 5) Nœud rectangulair text boxe
    text box/.style={
        rectangle,
        draw=white,
        fill=white,
    ...
    % Style pour les valeurs sur les arêtes
    edge value/.style={font=\scriptsize, auto},
    % Nœud textuel circulaire (ajouté selon vos instructions)
    text node/.style={
        circle, 
        draw=black, 
        fill=white, 
...
\DeclareCaptionType{pujoskaavio}[Pujoskaavio][List of Pujoskaaviot]
...
\newtheorem{konstruktio}[definition]{Konstruktio}
...
\newtheorem{vaite}[definition]{Väite}
...
\input{sections/1-johdanto}
\input{sections/2-differentiaaligeometria.tex}
\input{sections/3-algebralliset-käyrät.tex}
\input{sections/4-topologia.tex}
\input{sections/5-solmuteoriaa.tex}
\input{sections/6-pujoskaaviot.tex}
\input{sections/7-puiseux-sarjat.tex}
\input{sections/8-rpi-kaaviot.tex}
